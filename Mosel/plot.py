...
"""
This file reads the output from output.txt and plots the data using the networkx library.
"""
...
# Dictionary containing positional arguments for cities according to map
pos_og = {
    29: (1, 0),
    20: (1, 1),
    9: (1, 2),
...
# Reverse coordinates so that x-axis is south -> north and y-axis is east -> west
# for easier display
pos = {}
for key in pos_og:
    pos[key] = (pos_og[key][1], 8 - pos_og[key][0])

# Read data from output file (plotfile)
...
# Create graph, add edges and nodes
G = nx.Graph()
...
# Make color map depending on node type
color_map = []
for node in G:
...
e_l = {edge: bandwith[i] for i, edge in enumerate(sub_edges)}  # Add bandwith label to edges
plt.figure(figsize=(10, 5))
ax = plt.gca()
ax.set_title(f'Total cost: {objval}, Core-net cost: {corecost},Sub-net cost: {subcost}')  # Add costs to plot
...
labels = {i + 1: AllCities[i] for i in range(len(AllCities))}  # Add city names to plot

# Uncomment the line below to use autogenerated layout instead of positional coordinates
